...
const SignUpScreen : () => React$Node = (props) =>{
    ...
    useEffect(()=>{
        fetch('http://192.168.0.5:3000/get/terms').then(res=>res.json()).then(data=>setTerm(data.content))
    })
    ...
    const doSignUp = ()=>{
        if(doubleChk === false) errorToast('아이디 중복확인을 해주세요.')
            ...
            axios.post('http://192.168.0.5:3000/users/register',{id:id,name:name,phone:phone,pw:password2,email:mail})
            .then((response)=>{
    ...
    const doDoubleCheck = () =>{
        axios.post('http://192.168.0.5:3000/users/doubleCheck',{id:id})
        .then((response)=>{
            if(response.data.result==="success") {
