...
const FindPwForm = (props)=>{
    ...
    const onRequest = async()=>{
        setErrorText({text:"-",color:"#FFFFFF"});
        if(name === "" || target === "" || userId === "") setErrorText({text:"* 해당 정보를 입력해주세요",color:"#EE1818"})
        else {
            if(request === true){
        ...
        <>
            ...
            <View>
                <View style={{borderWidth:1,borderColor:"#B6B6B6",padding:16,borderRadius:6}}>
                    <TextInput placeholderTextColor={"#A7A7A7"} placeholder={"아이디를 입력해주세요"} onChangeText={text=>setUserId(text)} style={{padding:0,fontSize:13,color:"#2B2B2B",height:20,fontFamily:"NotoSans-Regular"}}/>
                </View>
